...
uint32_t NbPeriodicEventThreads = 0;
...
// **similar to Lab 3. initialize priority field****
  ...
  tcbs[4].next = &tcbs[5]; // 4 points to 5
	...
	// Initialize Blocked field to zero
	tcbs[0].blocked = 0 ; 
  tcbs[1].blocked = 0 ; 
  tcbs[2].blocked = 0 ; 
	tcbs[3].blocked = 0 ; 
  tcbs[4].blocked = 0 ; 
	...
	// Initialize Sleep field to zero
	tcbs[0].Sleep = 0 ; 
  tcbs[1].Sleep = 0 ; 
  tcbs[2].Sleep = 0 ; 
	tcbs[3].Sleep = 0 ; 
...
// In Lab 4, handle periodic events in RealTimeEvents
		for(int j=0;j<NbPeriodicEventThreads;j++){ // periodic threads scanned first
		if (pcbs[j].current_period == 1){
			pcbs[j].periodictask();
			pcbs[j].current_period =pcbs[j].period;
		}else{
			pcbs[j].current_period --;
		}
	}
...
// (d) PD6 is edge-sensitive
 GPIO_PORTD_IS_R &= ~0x40;  // PF4,PF0 are edge-sensitive 		
...
// (f) arm interrupt on PD6
		GPIO_PORTD_IM_R |= 0x40;   // arm interrupts on PF4,PF0
// priority on Port D edge trigger is NVIC_PRI0_R	31 – 29
  NVIC_PRI7_R = (NVIC_PRI7_R&0x00FFFFFF)|0xA0000000; 
